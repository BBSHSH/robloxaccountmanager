...
document.querySelector("#open-add").addEventListener("click", async () => { try { state.accounts = await window.launcher.addAndLogin(); render(); showToast("公式Robloxログイン画面を開きました。"); } catch (error) { showToast(error.message); } });
...
accountsElement.addEventListener("click", async (event) => { const target = event.target; const id = target.dataset.login || target.dataset.verify || target.dataset.open || target.dataset.gameOpen || target.dataset.remove; if (!id) return; try { if (target.dataset.login) { await window.launcher.login(id); showToast("公式Robloxログイン画面を開きました。ログイン完了後は自動でカードを更新します。"); } else if (target.dataset.verify) { const updated = await window.launcher.verify(id); state.accounts = state.accounts.map((account) => account.id === updated.id ? updated : account); render(); showToast(updated.loginStatus === "verified" ? "ログインを確認しました。" : "ログイン確認に失敗しました。"); } else if (target.dataset.open) { await window.launcher.open(id); showToast("Robloxホームを開きました。"); } else if (target.dataset.gameOpen) { await window.launcher.openGame(id, gameFor(id)); showToast("ゲームページを開きました。"); } else if (target.dataset.remove && confirm("この一覧から削除しますか？ 保存済みCookieも削除されます。")) { state.accounts = await window.launcher.removeAccount(id); render(); showToast("アカウントを削除しました。"); } } catch (error) { showToast(error.message); } });
window.launcher.onAccountCaptured((updated) => { state.accounts = state.accounts.map((account) => account.id === updated.id ? updated : account); render(); showToast(`${updated.displayName || updated.name} をログイン済みに更新しました。`); });
window.launcher.onAccountDiscarded((id) => { state.accounts = state.accounts.filter((account) => account.id !== id); render(); showToast("ログインが完了しなかったため、追加を取り消しました。"); });
reload();
